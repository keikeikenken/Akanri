...
class PushTimecard(LoginRequiredMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        is_attendanced = Attendance.objects.filter( 
            user = request.user,
            attendance_time__date = date.today()
        ).exists()
        
        is_left = Attendance.objects.filter(
             user = request.user,
     ...
     def get(self, request, *args, **kwargs):
         ...
         for attendance in month_attendances:
             ...
             if leave_time:
                 ...
                 if attendance_time.date() == today.date(): #leave_timeの値を整形するときの条件に追加として出勤時刻の日付と今日の日付が一致しているかどうかを比較する処理
                     leave_time = None
                 else:
